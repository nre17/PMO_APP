...
export interface WorkItem extends Versioned {
  ...
  stage: Stage; priority: Priority; ownerId: string; currentOwnerId: string;
...
export interface HubState { settings: Settings; members: Member[]; workstreams: Workstream[]; deliverables: Deliverable[]; milestones: Milestone[]; items: WorkItem[]; tests: TestResult[]; registers: Register[]; meetings: Meeting[]; submissions: Submission[]; reports: Report[]; events: AuditEvent[] }
