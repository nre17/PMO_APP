...
The demo runs on loopback with fictional personas and persistent local data. Follow [the README](../README.md) for configuration and restart behaviour. Use `pnpm demo:reset` only when intentionally starting a fresh synthetic workspace; ordinary restarts preserve edits.

1. Describe the user problem and acceptance examples in an issue or the pull request.
2. Make the change on the appropriate branch. Follow the approved [design plan](DESIGN-PLAN.md) and its linked references for UI work.
