...
**Work covers every lifecycle phase.** A discovery workshop, current-state assessment, scope presentation, data task or adoption follow-up can be captured directly, without a pre-existing tracker item or linked deliverable. New capture defaults to **General work / Action**. Backlog means the work has not started; it does not identify where the work came from. Each work item still belongs to a use case, and its work stage is separate from that use case's lifecycle phase.
