...
export function createShowcaseState(now = new Date()): HubState {
  ...
  type UseCaseKey = keyof typeof ids;
  const allKeys = Object.keys(ids) as UseCaseKey[];
  const member = (id: string, name: string, role: Member['role'], title: string, color: string, keys: UseCaseKey[]): Member => ({
    id, name, role, initials: name.split(' ').map(part => part[0]).join(''), title, color,
    workstreamIds: keys.map(key => ids[key]), canApproveReports: role === 'executive',
  });
  // Names and responsibilities are fictional. Stable IDs preserve saved tour links.
  state.members = [
    member('demo-pmo', 'Maya Haddad', 'pmo', 'Programme PMO', '#7941a8', allKeys),
    member('demo-lead', 'Omar Rahman', 'lead', 'Investment Delivery Lead', '#426b82', ['ppi', 'investment', 'mi', 'benchmark']),
    member('demo-engineer', 'Arjun Mehta', 'contributor', 'Frontend Engineer', '#446c65', ['ppi', 'mi', 'spreadsheet']),
    member('demo-qa', 'Priya Nair', 'contributor', 'Quality Assurance Lead', '#846a39', ['ppi', 'legal', 'mi', 'accounting', 'benchmark', 'treasury', 'spreadsheet']),
    member('demo-approver', 'Daniel Mercer', 'executive', 'Engagement Director', '#5b5677', allKeys),
    member('demo-finance-lead', 'Leila Mansour', 'lead', 'Finance Delivery Lead', '#805568', ['accounting', 'treasury', 'spreadsheet']),
    member('demo-strategy-lead', 'Nadia Saleh', 'lead', 'Strategy & Discovery Lead', '#59734c', ['legal', 'finance', 'budget']),
    member('demo-data-engineer', 'Youssef Farah', 'contributor', 'Data Engineer', '#52758a', ['ppi', 'benchmark', 'treasury', 'spreadsheet', 'finance']),
    member('demo-ai-engineer', 'Sofia Costa', 'contributor', 'AI & Backend Engineer', '#856146', ['investment', 'legal', 'accounting', 'treasury']),
    member('demo-analyst', 'Rami Khalil', 'contributor', 'Business Analyst', '#686593', ['investment', 'legal', 'finance', 'budget']),
  ];
  const person = (id: string) => state.members.find(value => value.id === id)!;
  const pmo = person('demo-pmo'), lead = person('demo-lead'), qa = person('demo-qa'), engineer = person('demo-engineer'), analyst = person('demo-analyst');
  const leads: Record<UseCaseKey, string> = { ppi: lead.id, investment: lead.id, mi: lead.id, benchmark: lead.id, accounting: 'demo-finance-lead', treasury: 'demo-finance-lead', spreadsheet: 'demo-finance-lead', legal: 'demo-strategy-lead', finance: 'demo-strategy-lead', budget: 'demo-strategy-lead' };
  const leadFor = (key: UseCaseKey) => person(leads[key]);
  const workstreamLead = (streamId: string) => person(state.workstreams.find(stream => stream.id === streamId)!.leadId);
  ...
  for (const [key, suffix, title, status] of artifacts) state.deliverables.push({ ...v(`showcase-artifact-${key}-${suffix}`, -8), title, workstreamId: ids[key], ownerId: leadFor(key).id, description: `Illustrative artifact. ${artifactContents[`${key}-${suffix}`]}`, lifecyclePhase: briefs[key].phase, status, evidenceLinks: [evidence(`artifact-${key}-${suffix}`)] });
  ...
  const specs: ItemSpec[] = [
    ...
    { id: 'showcase-mi-release', key: 'mi', artifact: 'release', title: 'Keep keyboard focus on the selected intelligence card', stage: 'Development', category: 'bug', due: 1, criteria: 'Tab order remains stable after selection, refresh and return from an article.', action: `Hand the implemented fix to ${qa.name} for UAT.` },
    ...
    { id: 'showcase-accounting-exceptions', key: 'accounting', artifact: 'release', title: 'Release the grouped validation-exception view', stage: 'Ready for production', due: 3, criteria: 'Grouping preserves the original row reference and exception count.', action: `Record deployment evidence and hand over to ${qa.name}.` },
  ...
  const implementer = (spec: ItemSpec): Member => {
    if (spec.id === 'showcase-budget-scope') return pmo;
    if (spec.category === 'data' || spec.key === 'benchmark') return person('demo-data-engineer');
    if (spec.kind === 'general') return analyst.workstreamIds.includes(ids[spec.key]) ? analyst : leadFor(spec.key);
    if (['legal', 'accounting', 'treasury', 'investment'].includes(spec.key)) return person('demo-ai-engineer');
    return engineer;
  };
  function pass(item: WorkItem, stamp: string) {
    const reviewer = item.kind === 'general' ? workstreamLead(item.workstreamId) : qa;
    const result = { id: `${item.id}-pass-${item.stage.toLowerCase().replaceAll(' ', '-')}`, itemId: item.id, cycle: item.cycle, stage: item.stage, result: 'pass' as const, blocking: false, notes: `Illustrative recorded pass: ${item.acceptanceCriteria}`, evidence: evidence(`${item.id}-${item.stage}`), authorId: reviewer.id, createdAt: stamp };
    state.tests.push(result); touch(item, stamp);
    audit(state, reviewer, 'tests', result.id, 'test_recorded', 'Illustrative review result with its acceptance criteria and evidence.', null, result, stamp);
  }
  for (const spec of specs) {
    ...
    for (let index = 1; index <= targetIndex; index++) {
      ...
      transition(state, pmo, item.id, { version: item.version, stage: nextStage, currentOwnerId: ['UAT', 'Production verification'].includes(nextStage) ? qa.id : ['Review', 'Awaiting client acceptance', 'Closed'].includes(nextStage) ? accountableLead.id : assigned.id, ...(nextStage === 'Production verification' ? { evidence: evidence(`${item.id}-deployment`) } : {}) }, stamp);
    }
    // Backlog records also received a recent owner review; age is not an abandoned assignment.
    if (item.stage === 'Backlog') { const before = structuredClone(item); touch(item, end); audit(state, person(ownerId), 'items', item.id, 'reviewed', 'Owner reviewed the next action and target date.', before, item, end); }
    if (item.kind === 'software' && spec.stage === 'Closed') {
      ...
      audit(state, accountableLead, 'items', item.id, 'client_accepted', `Illustrative client acceptance recorded by ${accountableLead.name}. Evidence: ${evidence(`${item.id}-client-response`)}`, before, item, end);
    }
  }
  const blocked = state.items.find(item => item.id === 'showcase-ppi-blocker')!;
  const priorBlocker = structuredClone(blocked);
  Object.assign(blocked, { blocked: true, blockedSince: at(-1), blockReason: 'Source access is awaiting the data-owner decision.', nextAction: `${lead.name} to obtain the sample-access decision and confirm a revised reconciliation date with ${person('demo-data-engineer').name}.` });
  ...
  const milestone = (id: string, title: string, keys: (keyof typeof ids)[], artifactIds: string[], baseline: number, forecast: number, status: 'planned' | 'at_risk' | 'complete' = 'planned') => ({ ...v(id), title, workstreamIds: keys.map(key => ids[key]), deliverableIds: artifactIds.map(value => `showcase-artifact-${value}`), ownerId: new Set(keys.map(key => leads[key])).size > 1 ? pmo.id : leadFor(keys[0]).id, baselineDate: date(baseline), forecastDate: date(forecast), status, notes: 'Illustrative commitment. The original baseline and current forecast are retained separately.', ...(status === 'complete' ? { actualDate: dateInTimezone(new Date(currentCloseTime), state.settings.timezone), updatedAt: currentCloseTime } : {}) });
  state.milestones = [
  ...
  state.registers = [
    ...
    { ...register('showcase-raid-internal', 'risk', 'investment', 'Internal pilot-support coverage needs confirmation', ['showcase-investment-adoption'], [], 'Confirm the support roster and escalation contact.', 'The pilot support roster still needs a named backup for analyst training and first-line issue triage.', 3), clientVisible: false, clientSummary: '' },
  ];
  state.meetings = [
    { ...v('showcase-meeting-daily', -2), title: 'Daily delivery review', heldAt: at(-2), notes: `Illustrative meeting: ${lead.name} follows up sample access with ${person('demo-data-engineer').name}. ${engineer.name} hands the keyboard fix to ${qa.name}. ${leadFor('accounting').name} follows up the accounting client response. Owners and next actions remain in the linked records.`, linkedItemIds: ['showcase-ppi-blocker', 'showcase-mi-release', 'showcase-accounting-acceptance'], linkedRegisterIds: ['showcase-raid-access', 'showcase-raid-client'] },
    { ...v('showcase-meeting-discovery', -12), title: 'Discovery synthesis review', heldAt: at(-12), notes: `Illustrative meeting: ${analyst.name} separates observations and assumptions in the budget synthesis. ${leadFor('budget').name} reviews the two scope options; the decision remains open. ${pmo.name} owns the existing scope follow-up so that a second tracker is unnecessary.`, linkedItemIds: ['showcase-budget-discovery', 'showcase-budget-scope'], linkedRegisterIds: ['showcase-raid-budget'] },
    { ...v('showcase-meeting-quality', -24), title: 'Assurance review', heldAt: at(-24), notes: `Illustrative meeting: ${qa.name} found the empty-result filter regression. ${person('demo-data-engineer').name} brings two reference-index mappings to ${lead.name} for decision. Resolve findings and record evidence before advancing the affected work.`, linkedItemIds: ['showcase-ppi-filter', 'showcase-benchmark-labels'], linkedRegisterIds: ['showcase-raid-filter', 'showcase-raid-labels'] },
  ];
  ...
  for (const row of state.sourceRecords!.filter(value => value.itemId)) {
    ...
    Object.assign(item, { ownerId: leadFor('finance').id, currentOwnerId: analyst.id, priority: 'Medium', baselineDate: date(row.sourceKey === 'DEMO-1' ? 2 : -1), dueDate: date(row.sourceKey === 'DEMO-1' ? 2 : -1), deliverableId: `showcase-artifact-finance-${row.sourceKey === 'DEMO-1' ? 'sources' : 'brief'}`, acceptanceCriteria: row.sourceKey === 'DEMO-1' ? 'Record the agreed contact role, feasibility discussion and next owner action.' : 'A reviewer records the current scope position and resolves the contradictory source remarks.', evidenceLinks: [evidence(`work-${item.id}`)], nextAction: row.sourceKey === 'DEMO-1' ? `Hold the source-feasibility discussion with ${person('demo-data-engineer').name}.` : 'Use the reviewed scope position in the opportunity brief.', clientVisible: true, clientSummary: row.title });
    ...
    transition(state, pmo, item.id, { version: item.version, stage: 'In progress', currentOwnerId: analyst.id }, at(-23));
    if (row.sourceKey === 'DEMO-2') {
      transition(state, pmo, item.id, { version: item.version, stage: 'Review', currentOwnerId: leadFor('finance').id }, at(-22));
      pass(item, at(-21));
      transition(state, pmo, item.id, { version: item.version, stage: 'Closed' }, at(-20));
    }
    const sourceBefore = structuredClone(row);
    row.resolutionNotes = row.sourceKey === 'DEMO-1' ? `${pmo.name} confirmed the new source contact. ${analyst.name} arranges the feasibility discussion, with ${leadFor('finance').name} accountable for the target date.` : `Review by ${leadFor('finance').name} confirmed the framing question was answered. The linked status-confirmation action passed review and closed; the original source label remains unchanged.`;
  ...
  for (const [index, daysAgo] of [56, 28].entries()) {
    ...
    for (const [key, streamId] of Object.entries(ids) as [keyof typeof ids, string][]) {
      ...
      history.submissions.push({ id: `showcase-submission-month-${index + 1}-${key}`, version: 1, updatedAt: archiveAt, workstreamId: streamId, periodEnd: historicalPeriod.end, completed: stream.statusNote, next: `Review the next ${phases[key][index]} commitment with ${leadFor(key).name}.`, changes: 'Monthly illustrative scenario reviewed.', blockers: 'No unresolved decision represented in this historical example.', health: 'green', confirmedBy: leadFor(key).id, confirmedAt: archiveAt, sourceUpdatedAt: latestWorkstreamChange(history, streamId) });
    }
  ...
  for (const [key, streamId] of Object.entries(ids)) {
    const stream = state.workstreams.find(value => value.id === streamId)!, confirmer = workstreamLead(streamId);
    const completed = state.items.filter(item => item.workstreamId === streamId && item.closedAt && item.closedAt >= range.startAt);
    const submission = { id: `showcase-submission-${key}`, version: 1, updatedAt: now.toISOString(), workstreamId: stream.id, periodEnd: range.end, completed: completed.length ? completed.map(item => item.title).join('; ') : `No closures this period. ${stream.statusNote}`, next: stream.nextGate!, changes: stream.statusNote, blockers: key === 'ppi' ? `The sample-access decision remains outstanding; ${pmo.name}'s review is requested.` : 'No additional blocker beyond the linked register records.', health: stream.health, confirmedBy: confirmer.id, confirmedAt: now.toISOString(), sourceUpdatedAt: latestWorkstreamChange(state, stream.id) };
    state.submissions.push(submission); audit(state, confirmer, 'submissions', submission.id, 'confirmed', `${confirmer.name} reviewed the current work, dates and concerns for this reporting period.`, null, submission, now.toISOString());
  }
  ...
  internal.body.summary = `Illustrative internal check-in: ${pmo.name} owns the Budget scope follow-up, ${lead.name} owns the portfolio access decision, and ${engineer.name} hands the Market Intelligence fix to ${qa.name}. Confirm pilot-support coverage before the next adoption readout. All ten use-case updates have been reviewed.`;
