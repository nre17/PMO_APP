...
export function MeetingWorkspace({ onClose, ...props }: PageProps & { onClose: () => void }) {
  ...
  const [editingRecord, setEditingRecord] = useState<Meeting | undefined>(original);
  const [title, setTitle] = useState(original?.title || 'Project working session');
  ...
  const stale = Boolean(editingRecord && original?.version !== editingRecord.version);
  const allowedStreams = state.workstreams.filter(stream => user.role === 'pmo' || user.role === 'admin' || user.workstreamIds.includes(stream.id) || (user.role === 'lead' && stream.leadId === user.id));
  ...
  async function save(): Promise<Meeting> {
    const data = { title, heldAt, notes, linkedItemIds: editingRecord?.linkedItemIds || [], linkedRegisterIds: editingRecord?.linkedRegisterIds || [] };
    const result: Meeting = await mutate(editingRecord ? `/api/records/meetings/${editingRecord.id}` : '/api/records/meetings', editingRecord ? { version: editingRecord.version, changes: data } : data, editingRecord ? 'PATCH' : 'POST');
  ...
  function selectMeeting(id: string) {
    ...
    setSelectedId(id); setTitle(meeting?.title || 'Project working session'); setNotes(meeting?.notes || ''); setHeldAt(meeting?.heldAt || new Date().toISOString()); setProposals([]); setError('');
  }

  return <Modal wide className="meeting-workspace" title="Project meetings" description="Capture discovery workshops, working sessions, and delivery reviews. Give each follow-up an owner." onClose={() => !busy && onClose()}>
    {error && <div className="notice notice-warning" role="alert">{error}</div>}
    {stale && <div className="notice notice-warning" role="status"><p>This meeting changed after you opened it. Your unsaved notes are still here. Copy anything you want to keep before reloading the latest record.</p><button className="button secondary small" disabled={busy} onClick={() => selectMeeting(selectedId)}>Reload latest notes</button></div>}
    <div className="meeting-layout">
      ...
      <section className="meeting-notes" aria-label="Meeting notes">
        ...
        {editable && <div className="form-actions meeting-save-row"><button className="button primary" disabled={busy || stale || !title.trim()} onClick={() => perform(async () => { await save(); notify('Meeting notes saved.'); })}><Save size={16} />{busy ? 'Working…' : 'Save notes'}</button><button className="button secondary" disabled={busy || stale || !notes.trim() || !title.trim() || !aiAvailable || !allowedStreams.length} title={!aiAvailable ? 'AI assistance is not configured. You can add follow-ups manually.' : 'Save notes and suggest follow-ups for your review.'} onClick={() => perform(async () => {
          await save();
    ...
    {editable && <section className="capture-section">
      ...
      {!proposals.length && <p className="capture-empty">{allowedStreams.length ? 'No pending follow-ups. Add an action, issue, or decision from the discussion.' : 'You need use case membership to capture a project follow-up.'}</p>}
      {proposals.map((proposal, index) => <article className="proposal-card" key={index}>
        ...
        <fieldset disabled={busy} style={{ border: 0, padding: 0, margin: 0, minWidth: 0 }}><div className="form-grid">
          ...
          <Field label="Use case"><select value={proposal.workstreamId} onChange={event => change(index, 'workstreamId', event.target.value)}>{allowedStreams.map(stream => <option value={stream.id} key={stream.id}>{stream.name}</option>)}</select></Field>
        ...
        <div className="form-actions"><button className="button ghost" disabled={busy} onClick={() => setProposals(current => current.filter((_, position) => position !== index))}>Discard</button><button className="button primary" disabled={busy || stale || !proposal.ownerId || !proposal.dueDate || !proposal.title.trim() || !title.trim()} onClick={() => perform(async () => {
          const saved = await save();
          const captured: { meeting: Meeting } = await mutate(`/api/meetings/${saved.id}/capture`, { version: saved.version, proposal, workstreamId: proposal.workstreamId });
          setEditingRecord(captured.meeting);
