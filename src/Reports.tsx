...
function sourceRecords(state: HubState) {
  return [
    ...state.workstreams.map(record => ({ ...record, key: `workstreams:${record.id}`, kind: 'Workstream', label: record.name, context: healthLabel[record.health], streams: [record.id] })),
    ...
    ...state.submissions.map(record => ({ ...record, key: `submissions:${record.id}`, kind: 'Confirmation', label: state.workstreams.find(stream => stream.id === record.workstreamId)?.name || 'Workstream confirmation', context: `Week ending ${formatDate(record.periodEnd)}`, streams: [record.workstreamId] })),
  ];
}
  ...
  return <div className="stack reporting-workspace">
    ...
    {view === 'current' ? <>
      ...
      <nav className="report-workflow" aria-label="Weekly report workflow">
        {[
          { title: 'Confirm workstreams', detail: `${confirmedCount} of ${streams.length} current`, done: confirmedCount === streams.length },
    ...
    <div className="report-desk">
      ...
      <aside className="report-review-sidebar" aria-label="Report review checklist">
        <section className="panel"><div className="panel-header"><h2 className="section-title">Review checklist</h2></div><div className="report-checklist">
          ...
          <div className={`report-check ${reviewMissing.length ? 'needs-attention' : 'complete'}`}>{reviewMissing.length ? <Clock3 size={18} /> : <CheckCircle2 size={18} />}<div><strong>{reviewMissing.length ? `${reviewMissing.length} confirmations outstanding` : 'Workstreams confirmed'}</strong><p>{reviewMissing.length ? reviewMissing.map(stream => stream.name).join(', ') : snapshotConfirmations ? 'Confirmed in this saved snapshot.' : 'All current workstream sources have been reviewed.'}</p>{snapshotConfirmations && <small>Status recorded for this edition.</small>}</div></div>
    ...
    {confirmation && <Modal title={`Confirm ${state.workstreams.find(stream => stream.id === confirmation.workstreamId)?.name || 'workstream'}`} description={`Review the internal position for the week ending ${formatDate(periodEnd)}.`} onClose={() => !busy && closeDialogs()} variant="drawer" className="report-confirmation-drawer" wide>
