...
export function Reports({ state, user, mutate, notify, aiAvailable, initialView = 'current', onCurrentPeriod, onViewChange }: PageProps & { initialView?: 'current' | 'archive'; onCurrentPeriod?:()=>void; onViewChange?:(view:'current'|'archive')=>void }) {
  ...
  return <div className="stack reporting-workspace">
    <div className="page-header">
      ...
      <div className="toolbar"><button className="button secondary" onClick={() => { if(onViewChange){onViewChange(view==='current'?'archive':'current');return;} if(view==='archive'&&initialView==='archive'&&onCurrentPeriod){onCurrentPeriod();return;} setView(view === 'current' ? 'archive' : 'current'); setArchiveStatus('all'); setSelectedId(null); setStep(null); setError(''); }}>{view === 'current' ? <History size={16} /> : <ArrowLeft size={16} />}{view === 'current' ? 'Previous editions' : 'Current period'}</button>{view === 'current' && isPMO && <button className="button primary" disabled={busy} onClick={prepare}><FileText size={16}/>New draft</button>}</div>
    </div>
