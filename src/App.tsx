...
function Workspace() {
  ...
  useEffect(()=>{const refresh=()=>{if(document.visibilityState==='visible')load().catch(()=>{});};refresh();const change=()=>{const route=currentRoute();setPage(route.page);setWorkView(route.workView);setReportView(route.reportView);window.scrollTo({top:0});};window.addEventListener('hashchange',change);window.addEventListener('focus',refresh);const timer=setInterval(refresh,60000);return()=>{window.removeEventListener('hashchange',change);window.removeEventListener('focus',refresh);clearInterval(timer);};},[load]);
